...
g=e.replacementOri,k=process.cwd(),z="",C="",m="",t="",u="",w=new Function("fs","globs","_pipe","_text","_find","_file","_path","_filename","_name","_ext","_cwd","code_rr","return eval(code_rr)");if(!e.dataIsPiped){z=r.normalize(r.join(process.cwd(),a));var A=r.parse(z);C=A.dir;m=A.base;t=A.name;u=A.ext}/\$\d/.test(e.replacement)?e.replacement=function(){var a=arguments;c(arguments);for(var b=z,p=C,e=m,n=t,v=u,r="",q=0;q<arguments.length-2;q++)r+="var $"+q+"\x3d"+JSON.stringify(a[q])+"; ";return w(l,
D,h,d,f,b,p,e,n,v,k,r+g)}:e.replacement=w(l,D,h,d,f,z,C,m,t,u,k,g)}var q=p.replace(e.regex,e.replacement);if(!e.outputMatch){if(e.output)return b("Output result from: "+a),process.stdout.write(q);if(q===p)v("Nothing changed in: "+a);else{p=void 0;b("Write new content to: "+a);if(e.voidBackup)return l.writeFile(a,q,e.encoding,function(b){if(b)return n(b);x(a)});var y=r.normalize(r.join(process.cwd(),a));p=(new Date).toISOString().toString().replace(/:/g,"_").replace("Z","");var B=y+"."+p+".backup";
if(e.voidAsync){try{l.renameSync(y,B),l.writeFileSync(y,q,e.encoding),e.keepBackup||l.unlinkSync(B)}catch(H){return n(H)}return x(a)}l.rename(y,B,function(b){if(b)return n(b);l.writeFile(y,q,e.encoding,function(b){if(b)return n(b);e.keepBackup?x(a):l.unlink(B,function(b){if(b)return n(b);x(a)})})})}}}k=a;c("Displaying steps for:");c(a);a.pattern=function(a){c("Get final pattern");a=a.pattern;c(a);return a}(a)||"";a.replacement=function(a){c("Get final replacement");if(a.replacementPipe){c("Piping replacement");
a.pipedDataUsed=!0;if(null===a.pipedData)return t("No data piped into replacement");a.replacement=a.pipedData}if(a.outputMatch)return c("Output match"),"6">process.versions.node?t("outputMatch is only supported in node 6+"):function(){var a=arguments;c(arguments);if(3===arguments.length)return c("Printing full match"),process.stdout.write(arguments[0]+"\n"),"";for(var b=1;b<arguments.length-2;b++)process.stdout.write(a[b]);process.stdout.write("\n");return""};if(a.replacementJs&&/\$\d/.test(a.replacement)&&
"6">process.versions.node)return t("Captured groups for javascript replacement is only supported in node 6+");c(a.replacement);return a.replacement}(a)||"";a.replacementOri=a.replacement;a.regex=function(a){c("Get final regex");var b=null;c("Get flags");var h="";a.voidGlobal||(h+="g");a.voidIgnoreCase||(h+="i");a.voidMultiline||(h+="m");a.unicode&&(h+="u");c(h);try{b=new RegExp(a.pattern,h)}catch(G){t("Wrongly formatted regex pattern",G)}c(b);return b}(a)||"";c(a);if(function(a){c("Check Piped Data");
return a.files.length?(a.replacementJs||v("Piped data never used."),!1):null===a.pipedData||a.pipedDataUsed?!1:(a.dataIsPiped=!0,a.output=!0)}(a))return d("Piped data",a,a.pipedData);a.files=D.sync(a.files);if(!a.files.length)return n(a.files.length+" files found");v(a.files.length+" files found");c(a);a.files.filter(function(a){return l.existsSync(a)?!0:n("File not found:",a)}).forEach(function(b){return p(b,a)})}var g={};g.red=g.green=g.gray=function(a){return a};var k=null,x=function(a,b){void 0===
b&&(b="");k.quiet||k.quietTotal||console.error(g.gray(a),b)},v=function(a,c){void 0===c&&(c="");k.verbose?x(a,c):b(a+" "+c)},t=function(a,b,c){void 0===b&&(b="");void 0===c&&(c=!1);c&&!k.quietTotal&&k.showHelp();n(a,b);f(a)},n=function(a,b){void 0===b&&(b="");k.quiet||k.quietTotal||console.error(g.red(a),b);k.halt&&f(a);return!1},l=require("fs"),r=require("path"),D=require("globs"),E=!1;if(4>process.argv.length)E=!0;else{var F=process.argv.splice(2,2);var w=F[0];var u=F[1]}var m=require("yargs").strict().usage("RexReplace 4.0.1: Regexp search and replace for files using lookahead and backreference to matching groups in the replacement. Defaults to global multiline case-insensitive search.\n\n\x3e rexreplace pattern replacement [fileGlob|option]+").example("\x3e rexreplace 'Foo' 'xxx' myfile.md",
"'foobar' in myfile.md will become 'xxxbar'").example("").example("\x3e rr Foo xxx myfile.md","The alias 'rr' can be used instead of 'rexreplace'").example("").example("\x3e rexreplace '(f?(o))o(.*)' '$3$1\u20ac2' myfile.md","'foobar' in myfile.md will become 'barfoo'").example("").example("\x3e rexreplace '^#' '##' *.md","All markdown files in this dir got all headlines moved one level deeper").version("v","Print rexreplace version (can be given as only argument)","4.0.1").alias("v","version").boolean("V").describe("V",
