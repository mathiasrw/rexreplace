...
(function () {
	...
	function engine(config) {
	    ...
	    function doReplacement(_file_rr, _config_rr, _data_rr) {
	        ...
	        if (_config_rr.replacementJs) {
	            ...
	            var _file_1 = '', _pathInfo_1 = '', _path_1 = '', _filename_1 = '', _name_1 = '', _ext_1 = '', dynamicContent_1 = new Function('_fs', '_globs', '_pipe', '_text', '_find', '_file', '_pathInfo', '_path', '_filename', '_name', '_ext', '_cwd', 'code_rr', 'return eval(code_rr)');
	            if (!_config_rr.dataIsPiped) {
	                _file_1 = path.normalize(path.join(process.cwd(), _file_rr));
	                _pathInfo_1 = path.parse(_file_1);
	                _path_1 = path.dir;
	                _filename_1 = path.base;
	                _name_1 = path.name;
	                _ext_1 = path.ext;
	            }
	            // Run only once if no captured groups (replacement cant change)
	            if (!/\$\d/.test(_config_rr.replacement)) {
	                _config_rr.replacement = dynamicContent_1(_fs_1, _globs_1, _pipe_1, _text_1, _find_1, _file_1, _pathInfo_1, _path_1, _filename_1, _name_1, _ext_1, _cwd_1, code_rr_1);
	            }
	            else {
	                ...
	                _config_rr.replacement = function () {
	                    ...
	                    return dynamicContent_1(_fs_1, _globs_1, _pipe_1, _text_1, _find_1, _file_1, _pathInfo_1, _path_1, _filename_1, _name_1, _ext_1, _cwd_1, capturedGroups + code_rr_1);
	                };
	            }
	        }
	...
	if (process.argv.length < 4) {
	    ...
	    _a = process.argv.splice(2, 2), pattern = _a[0], replacement = _a[1];
	}
	var yargs = require('yargs')
	    ...
	    .alias('u', 'unicode')["default"]('e', 'utf8')
	    .alias('e', 'encoding')
